...
type Profile = { kyc_tier: number; full_name: string; date_of_birth: string; address_line_1: string; city: string; country: string; };
...
export default function VerifyPage() {
  ...
  const fetchProfile = useCallback(async () => {
    ...
    if (user) {
      const { data, error } = await supabase.from('profiles').select('kyc_tier, full_name, date_of_birth, address_line_1, city, country').eq('id', user.id).single();
      if (error) setMessage(`Error fetching profile: ${error.message}`);
      else { setProfile(data as Profile); setFullName(data.full_name || ''); setDob(data.date_of_birth || ''); }
    }
    setLoading(false);
  }, []);
  ...
  const handleTier1Submit = async () => {
    ...
    const { data: { user } } = await supabase.auth.getUser();
    if (user) {
      const { error } = await supabase.from('profiles').update({ full_name: fullName, date_of_birth: dob, kyc_tier: 1 }).eq('id', user.id);
      if (error) setMessage(`Update failed: ${error.message}`);
      else { setMessage('Profile updated! You are now Tier 1.'); await fetchProfile(); }
    }
    setLoading(false);
  };
  
  const handleTier2Submit = async () => {
    if (!document) { setMessage('Please select a document to upload.'); return; }
    setLoading(true);
    setMessage('Uploading document...');
    const { data: { user } } = await supabase.auth.getUser();
    if (!user) { setLoading(false); return; }
    const filePath = `${user.id}/${document.name}`;
    const { error: uploadError } = await supabase.storage.from('kyc-documents').upload(filePath, document, { upsert: true });
    if (uploadError) { setMessage(`Upload failed: ${uploadError.message}`); setLoading(false); return; }
    setMessage('Document uploaded. Submitting for verification...');
    setTimeout(async () => {
        const { error: tierError } = await supabase.from('profiles').update({ kyc_tier: 2 }).eq('id', user.id);
        if (tierError) setMessage(`Verification submission failed: ${tierError.message}`);
        else { setMessage('Verification successful! You are now Tier 2.'); await fetchProfile(); }
        setLoading(false);
    }, 3000);
  };

  if (loading) return <p>Loading your verification status...</p>;
